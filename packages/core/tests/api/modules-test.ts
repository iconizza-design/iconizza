import type { IconizzaAPIConfig } from '../../lib/api/config';
...
describe('Testing API modules', () => {
	...
	const prepareQuery = (
		...
		const item: IconizzaAPIIconsQueryParams = {
			type: 'icons',
			provider,
			prefix,
			icons,
	...
	it('Empty module', () => {
		...
		// Get config
		const config = getAPIConfig(provider) as IconizzaAPIConfig;
		...
		// Check getAPIModule()
		const item = getAPIModule(provider) as IconizzaAPIModule;
