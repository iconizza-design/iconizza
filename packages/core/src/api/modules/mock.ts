 ...
 */
export type IconizzaMockAPIDelayDoneCallback = () => void;
export type IconizzaMockAPIDelayCallback = (
	next: IconizzaMockAPIDelayDoneCallback
) => void;

/**
 * Fake API result
 */
interface IconizzaMockAPIBase {
	// Response
	// Number if error should be sent, JSON on success
	response: number | IconizzaJSON | Record<string, unknown>;

	// Delay for response: in milliseconds or callback
	delay?: number | IconizzaMockAPIDelayCallback;
}

export interface IconizzaMockIconsAPI extends IconizzaMockAPIBase {
	type: 'icons';
	provider: string;
	...
	// Response
	// Number if error should be sent, JSON on success
	response: number | IconizzaJSON;
}

export interface IconizzaMockCustomAPI extends IconizzaMockAPIBase {
	type: 'custom';
	provider: string;
...
export interface IconizzaMockCustomHostAPI extends IconizzaMockAPIBase {
	type: 'host';
	host: string;
...
export type IconizzaMockAPI =
	| IconizzaMockIconsAPI
	| IconizzaMockCustomAPI
	| IconizzaMockCustomHostAPI;
...
export const iconsStorage: Record<
	string,
	Record<string, IconizzaMockIconsAPI[]>
> = {};
...
export const customProviderStorage: Record<
	string,
	Record<string, IconizzaMockCustomAPI>
> = {};
...
export const customHostStorage: Record<
	string,
	Record<string, IconizzaMockCustomHostAPI>
> = {};

/**
 * Set data for mocking API
 */
export function mockAPIData(data: IconizzaMockAPI): void {
	switch (data.type) {
		case 'icons': {
		...
		case 'custom': {
			...
			const providerStorage =
				...
				(customProviderStorage[provider] = Object.create(
					null
				) as Record<string, IconizzaMockCustomAPI>);
...
interface MockAPIIconsQueryParams extends IconizzaAPIIconsQueryParams {
	index: number;
}
 ...
 * Return API module
 */
export const mockAPIModule: IconizzaAPIModule = {
	/**
	 * Prepare params
	 */
	prepare: (
		...
		// Sort results
		const results: IconizzaAPIIconsQueryParams[] = [];
		if (noMatch.length > 0) {
			results.push({
				type,
				provider,
		...
		Object.keys(matches).forEach((key) => {
			...
			results.push({
				...
				icons: matches[index],
				index,
			} as IconizzaAPIIconsQueryParams);
		});
	...
	send: (
		...
		let callback: IconizzaMockAPIDelayCallback;
		switch (typeof delay) {
			case 'function':
				callback = delay;
				break;
