import type { IconizzaIconBuildResult } from '../lib/svg/build';
import { iconToSVG } from '../lib/svg/build';
import type { IconizzaIcon } from '../lib/icon/defaults';
import type { IconizzaIconCustomisations } from '../lib/customisations/defaults';
import { iconToHTML } from '../lib/svg/html';

describe('Testing iconToSVG', () => {
	test('Empty icon', () => {
		const icon: IconizzaIcon = { body: '' };
		const expected: IconizzaIconBuildResult = {
			attributes: {
				width: '1em',
				height: '1em',
				viewBox: '0 0 16 16',
	...
	test('Auto size, body', () => {
		const custom: IconizzaIconCustomisations = {
			height: 'auto',
		};
