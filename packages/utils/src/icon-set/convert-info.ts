...
/**
 * Item provided by API or loaded from collections.json, slightly different from IconizzaInfo
 */
export interface LegacyIconizzaInfo {
   // Icon set name.
   name: string
...
export function convertIconSetInfo(
   ...
   switch (typeof source.total) {
      ...
      case 'string': {
         const num = Number.parseInt(source.total)
         if (num > 0)
            info.total = num
   ...
   if (
      ...
      const num = Number.parseInt(source.height as string)
      if (num > 0)
         info.height = num
   }

   if (Array.isArray(source.height)) {
      source.height.forEach((item) => {
         const num = Number.parseInt(item as string)
         if (num > 0) {
            if (!(Array.isArray(info.height)))
               info.height = []
   ...
   switch (typeof source.palette) {
      ...
      case 'string':
         switch (source.palette.toLowerCase()) {
            case 'colorless': // Iconizza v1
            case 'false': // Boolean as string
               info.palette = false
               break

            case 'colorful': // Iconizza v1
