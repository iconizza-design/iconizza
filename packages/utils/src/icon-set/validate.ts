...
/**
 * Validate icon set, return it as IconizzaJSON type on success, throw error on failure
 */
export function validateIconSet(
   ...
   for (const name in tree) {
      ...
      if (!isAlias) {
         // Check body
         if (typeof (item as ExtendedIconizzaIcon).body !== 'string') {
            if (fix) {
               delete parentObj[name]
               continue
            }
   ...
   // Check not_found
   if (data.not_found !== void 0 && !(Array.isArray(data.not_found))) {
      if (fix)
