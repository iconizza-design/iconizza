...
function rgb2hsl(rgb: RGBColor): HSLColor {
   ...
   const c2 = rgb.g / 255
   const c3 = rgb.b / 255
   const kmin = Math.min(c1, Math.min(c2, c3))
   const kmax = Math.max(c1, Math.max(c2, c3))
   const l = (kmax + kmin) / 2
...
function fromFunction(value: string): Color | null {
   ...
   if (typeof alphaStr === 'string') {
      alpha = Number.parseFloat(alphaStr)
   ...
   for (let i = 0; i < 3; i++) {
      ...
      const colorNum = Number.parseFloat(colorStr)
      if (isNaN(colorNum))
         return null
...
function fromHex(value: string): Color | null {
   ...
   switch (value.length) {
      case 4: {
         // rgba
         alpha = Number.parseInt(value[3] + value[3], 16) / 255
      }
      ...
      case 8: {
         alpha = Number.parseInt(value[6] + value[7], 16) / 255
      }
