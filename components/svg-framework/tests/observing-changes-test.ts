import { iconLoaded } from '@iconizza/core/lib/storage/functions';
import {
	fakeAPI,
	nextPrefix,
	setupDOM,
...
describe('Observing DOM changes', () => {
	...
	it('Loading icon, transforming icon', async () => {
		...
		setupDOM(`<span class="iconizza" data-icon="${iconName}"></span>`);
		await waitDOMReady();
		...
		expect(document.body.innerHTML).toBe(
			`<span class="iconizza" data-icon="${iconName}"></span>`
		);
		...
		// Check HTML
		expect(document.body.innerHTML).toBe(
			`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 16 16" data-icon="${iconName}" class="iconizza iconizza--${provider} iconizza--${prefix}"><g></g></svg>`
		);
		...
		expect(document.body.innerHTML).toBe(
			`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 16 16" data-icon="${iconName}" data-rotate="90deg" class="iconizza iconizza--${provider} iconizza--${prefix}"><g transform="rotate(90 8 8)"><g></g></g></svg>`
		);
	});
	...
	it('Changing icon name after rendering first icon', async () => {
		...
		// Check HTML
		expect(document.body.innerHTML).toBe(
			`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 16 16" data-icon="${iconName}" class="iconizza iconizza--${provider} iconizza--${prefix1}"><g></g></svg>`
		);
		const svg = document.body.childNodes[0] as SVGSVGElement;
		const svgData = svg[elementDataProperty];
		expect(svgData.status).toBe('loaded');
		...
		expect(document.body.innerHTML).toBe(
			`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 16 16" data-icon="${iconName2}" class="iconizza iconizza--${provider} iconizza--${prefix1}"><g></g></svg>`
		);
		...
		expect(document.body.innerHTML).toBe(
			`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 24 24" data-icon="${iconName2}" class="iconizza iconizza--${provider} iconizza--${prefix2}"><path d="M0 0v2"></path></svg>`
		);
	});
	...
	it('Changing icon name while loading first icon', async () => {
		...
		expect(document.body.innerHTML).toBe(
			`<span class="iconizza" data-icon="${iconName2}"></span>`
		);
