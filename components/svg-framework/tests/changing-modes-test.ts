...
describe('Changing render modes', () => {
	...
	it('Various background modes', async () => {
		...
		setupDOM(
			`<span class="iconizza" data-icon="${iconName}" data-mode="style"></span>`
		);
		await waitDOMReady();
		...
		expect(document.body.innerHTML).toBe(
			`<svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1em" height="1em" viewBox="0 0 16 16" data-icon="${iconName}" data-mode="inline" style="" class="iconizza iconizza--${provider} iconizza--${prefix}"><g></g></svg>`
		);
