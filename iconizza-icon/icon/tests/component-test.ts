...
import { defineIconizzaIcon, IconizzaIconHTMLElement } from '../src/component';
import type { IconState } from '../src/state';

export declare interface DebugIconizzaIconHTMLElement
	extends IconizzaIconHTMLElement {
...
describe('Testing icon component', () => {
	...
	it('Registering component', () => {
		...
		expect(window.customElements.get('iconizza-icon')).toBeUndefined();

		// Define component
		const IconizzaIcon = defineIconizzaIcon();
		expect(IconizzaIcon).toBeDefined();
		expect(window.customElements.get('iconizza-icon')).toBeDefined();

		// Create element
		const node = document.createElement(
			'iconizza-icon'
		) as DebugIconizzaIconHTMLElement;
		expect(node instanceof IconizzaIcon).toBe(true);
		expect(node.status).toBe('loading');

		// Define component again (should return previous class)
		const IconizzaIcon2 = defineIconizzaIcon();
		expect(IconizzaIcon2).toBe(IconizzaIcon);

		// Create another element
		const node2 = document.createElement(
			'iconizza-icon'
		) as DebugIconizzaIconHTMLElement;
		expect(node2 instanceof IconizzaIcon).toBe(true);
	...
	it('Creating component instance, changing properties', async () => {
		...
		expect(typeof IconizzaIcon.loadIcon).toBe('function');
	...
	it('Restarting animation', async () => {
		...
		// Define component
		expect(defineIconizzaIcon()).toBeDefined();
		expect(window.customElements.get('iconizza-icon')).toBeDefined();
